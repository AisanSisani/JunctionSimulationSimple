...
namespace JSSimge.Som
{
  public class CTLightOC : HlaObjectClass
  {
    ...
    public CTLightOC() : base()
    {
      ...
      ClassPS = PSKind.Publish;
      
      // Create Attributes
      // tlight_id
      tlight_id = new HlaAttribute("tlight_id", PSKind.Publish);
      Attributes.Add(tlight_id);
      // state
      state = new HlaAttribute("state", PSKind.Publish);
      Attributes.Add(state);
      // duration_red
      duration_red = new HlaAttribute("duration_red", PSKind.Publish);
      Attributes.Add(duration_red);
      // duration_green
      duration_green = new HlaAttribute("duration_green", PSKind.Publish);
      Attributes.Add(duration_green);
      // belong_area
      belong_area = new HlaAttribute("belong_area", PSKind.Publish);
